       ...
       public static void main(String[] args) throws IOException {
         ...
         try {
			...
			try {
				...
				while (true)
			        {
			            String inp = clientSocket.getInetAddress().getHostAddress() + "|" + SockNum;
				...
				//SThread t = new SThread(RoutingTable, clientSocket, ind,2//, peerAddress, peerNum
				//		); // creates a thread with a random port
				//t.start(); // starts the thread
				ind++; // increments the index
				System.out.println("ServerRouter2 connected with Client/Server: " + clientSocket.getInetAddress().getHostAddress());
				break;
         }
             catch (IOException e) {
